#!/usr/bin/env python3
"""
Medical Term Embedding Training using GPT with Hybrid Loss
...
class MedicalGPTEmbedder(nn.Module):
    def __init__(self, vocab_size=50257, n_layers=2, n_embed=128, n_heads=2, n_positions=32, 
        ...
        # TransE relation embeddings (only used if use_transe=True)
        self.relation_embeddings = nn.Embedding(num_relations, n_embed)
    ...
    def forward(self, head_ids, head_mask, tail_ids, tail_mask, relation_indices=None):
        ...
        # TransE: get relation embeddings if needed
        relation_emb = None
...
def compute_cosine_loss(head_emb, tail_emb, labels_for_cosine):
    """Cosine Embedding Loss - keeps related terms close"""
    criterion_cosine = nn.CosineEmbeddingLoss()
    return criterion_cosine(head_emb, tail_emb, labels_for_cosine)


def compute_transE_loss(head_emb, relation_emb, tail_emb, margin=1.0):
    """
    TransE Loss - relationship as translation in vector space
    
    The idea: head + relation ≈ tail
    So: head + relation - tail should be close to 0 for true triples
    
    Loss = ||head + relation - tail||² (for positive)
    Loss = max(0, margin - ||head + relation - tail||²) (for negative)
    """
    # Compute translation: head + relation - tail
    translation = head_emb + relation_emb - tail_emb
    
    # L2 norm squared
    distance = torch.sum(translation ** 2, dim=1)
    
    # For positive samples (label=1), we want distance close to 0
    # For negative samples (label=-1), we want distance large
    
    # Simplified margin-based loss
...
def compute_hybrid_loss(head_emb, tail_emb, relation_emb, labels, cosine_weight=1.0, transe_weight=1.0):
    """
    Combined loss: Cosine + TransE
    
    - Cosine: pulls related terms closer (semantic similarity)
    - TransE: models relationships as translations (structural)
    """
    # Cosine loss
...
def train_epoch(model, dataloader, optimizer, tokenizer, category_to_idx, relation_to_idx, device, loss_type="hybrid", use_amp=True, grad_accum_steps=4):
    model.train()
    total_loss = 0
    total_cosine = 0
    total_transe = 0
        ...
        if use_amp:
            with autocast():
                ...
                if loss_type == "cosine":
                    ...
                    total_cosine += link_loss.item()
                    
                elif loss_type == "transe":
                    transe_loss = compute_transE_loss(head_emb, relation_emb, tail_emb)
                    loss = rel_loss + transe_loss * 3 + cat_loss
                    total_transe += transe_loss.item()
                    ...
                    loss = rel_loss + hybrid_loss + cat_loss
                    total_cosine += cos_loss
                    total_transe += trans_loss
    ...
    return total_loss / n_batches, total_cosine / n_batches, total_transe / n_batches
...
def main():
    ...
    print("="*60)
    print("MEDICAL EMBEDDINGS WITH HYBRID LOSS (GPT + TransE)")
    print("="*60)
    print(f"Loss Type: {LOSS_TYPE}")
    
    print("\nLoading data...")
    dataset = MedicalKGDataset("/home/harshal/medical-embeddings/medical_kg_enhanced.json")
    ...
    print("\nInitializing GPT model with TransE support...")
    ...
    for epoch in range(EPOCHS):
        ...
        avg_loss, avg_cosine, avg_transe = train_epoch(
            model, dataloader, optimizer, tokenizer, 
            category_to_idx, relation_to_idx, DEVICE, LOSS_TYPE
        )
        
        print(f"Average total loss: {avg_loss:.4f}")
        if LOSS_TYPE in ["cosine", "hybrid"]:
            print(f"Average cosine loss: {avg_cosine:.4f}")
        if LOSS_TYPE in ["transe", "hybrid"]:
            print(f"Average TransE loss: {avg_transe:.4f}")
    
    print("\nExtracting embeddings...")
    embeddings = extract_embeddings(model, dataset.term_list, tokenizer, DEVICE)
    
    # Determine output filename based on loss type
    if LOSS_TYPE == "cosine":
        output_path = "/home/harshal/medical-embeddings/medical_embeddings_gpt_enhanced.json"
    elif LOSS_TYPE == "transe":
        output_path = "/home/harshal/medical-embeddings/medical_embeddings_gpt_transe_enhanced.json"
    else:
        output_path = "/home/harshal/medical-embeddings/medical_embeddings_gpt_hybrid_enhanced.json"
